...
function SignalsScreen({ accent }) {
  const accentColor = ACCENTS[accent].main;
  return (
    <div style={{ padding: 20, height: '100%', overflow: 'auto' }}>
      <Label style={{ marginBottom: 14 }}>All signals · last 24h</Label>
      <Card noPad>
        <div style={{ padding: '12px 18px', borderBottom: `1px solid ${TOKENS.line}`, display: 'grid', gridTemplateColumns: '80px 60px 80px 1fr 80px 60px 100px', gap: 16, fontFamily: TOKENS.sans, fontSize: 10, color: TOKENS.ink3, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          <span>Symbol</span><span>Side</span><span>Score</span><span>Strategy</span><span>Urgency</span><span>Verdict</span><span>Time</span>
        </div>
        {[...DATA.conviction, ...DATA.conviction.slice(0,4).map(c => ({...c, sym: c.sym+'·'}))].map((c, i) => (
          <div key={i} style={{ padding: '12px 18px', borderBottom: `1px solid ${TOKENS.line}`, display: 'grid', gridTemplateColumns: '80px 60px 80px 1fr 80px 60px 100px', gap: 16, alignItems: 'center' }}>
            <span style={{ fontFamily: TOKENS.sans, fontSize: 13, fontWeight: 500, color: TOKENS.ink0 }}>{c.sym}</span>
            <span style={{ fontFamily: TOKENS.mono, fontSize: 11, color: c.side === 'short' ? TOKENS.loss : TOKENS.ink2 }}>{c.side}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ flex: 1, height: 3, background: 'rgba(255,255,255,0.05)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: `${c.score * 100}%`, height: '100%', background: c.side === 'short' ? TOKENS.loss : accentColor }}/>
              </div>
              <span style={{ fontFamily: TOKENS.mono, fontSize: 11, color: c.side === 'short' ? TOKENS.loss : accentColor, width: 30, textAlign: 'right' }}>{c.score.toFixed(2)}</span>
            </div>
...
function BookScreen({ accent }) {
  const accentColor = ACCENTS[accent].main;
  const totalPnl = DATA.positions.reduce((s, p) => s + p.pnl, 0);
  return (
    <div style={{ padding: 20, display: 'grid', gap: 14, gridTemplateColumns: '1fr 320px', height: '100%', overflow: 'hidden' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, minHeight: 0 }}>
        <Card>
          <Label style={{ marginBottom: 12 }}>Open positions</Label>
          <div style={{ display: 'grid', gap: 2 }}>
            {DATA.positions.map((p, i) => (
              <div key={p.sym} style={{ display: 'grid', gridTemplateColumns: '100px 70px 90px 90px 1fr 90px', gap: 12, alignItems: 'center', padding: '10px 0', borderBottom: `1px solid ${TOKENS.line}` }}>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontFamily: TOKENS.mono, fontSize: 10, color: TOKENS.ink3, width: 36, textAlign: 'right' }}>{(p.w*100).toFixed(0)}%</span>
                </div>
        ...
        <Card>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <div style={{ fontFamily: TOKENS.sans, fontSize: 24, fontWeight: 300, color: totalPnl >= 0 ? TOKENS.profit : TOKENS.loss, letterSpacing: '-0.02em' }}>
                {totalPnl >= 0 ? '+' : '−'}£{Math.abs(totalPnl).toFixed(2)}
              </div>
            </div>
            <div style={{ borderTop: `1px solid ${TOKENS.line}`, paddingTop: 10 }}>
              <Label style={{ marginBottom: 6 }}>Exposure</Label>
              {[['gross', DATA.exposure.gross], ['net', DATA.exposure.net], ['cash', DATA.exposure.cash]].map(([k, v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontFamily: TOKENS.mono, fontSize: 11 }}>
...
function AssetBreakdown({ accent }) {
  const classes = [
  ...
  return (
    <>
      <div style={{ display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', marginBottom: 10 }}>
        {classes.map(c => <div key={c.name} style={{ width: `${c.w*100}%`, background: c.color }}/>)}
      </div>
      {classes.map(c => (
        <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontFamily: TOKENS.mono, fontSize: 11 }}>
...
function RiskScreen({ accent }) {
  const accentColor = ACCENTS[accent].main;
  return (
    <div style={{ padding: 20, display: 'grid', gap: 14, gridTemplateColumns: '1fr 1fr', height: '100%', overflow: 'auto' }}>
      <Card>
        ...
        {DATA.approved.map((a, i) => (
          <div key={i} style={{ padding: '10px 0', borderBottom: `1px solid ${TOKENS.line}` }}>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 6, fontFamily: TOKENS.mono, fontSize: 11, color: TOKENS.ink3 }}>
      ...
      <Card style={{ gridColumn: '1 / -1' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }}>
          {[
            { name: 'Max drawdown', v: 0.32, cap: 1.0, tone: 'profit' },
            { name: 'Position heat', v: 0.68, cap: 1.0, tone: 'caution' },
            { name: 'Asset class cap', v: 0.95, cap: 1.0, tone: 'danger' },
            { name: 'Order rate', v: 0.21, cap: 1.0, tone: 'profit' },
          ].map(g => (
            <div key={g.name}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: TOKENS.mono, fontSize: 11, marginBottom: 4 }}>
                <span style={{ color: TOKENS.ink2 }}>{g.name}</span>
                <span style={{ color: g.tone === 'danger' ? TOKENS.danger : g.tone === 'caution' ? TOKENS.caution : TOKENS.profit }}>{(g.v*100).toFixed(0)}%</span>
              </div>
              <div style={{ height: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: `${g.v*100}%`, height: '100%', background: g.tone === 'danger' ? TOKENS.danger : g.tone === 'caution' ? TOKENS.caution : accentColor, transition: `width 600ms ${TOKENS.ease}` }}/>
              </div>
            </div>
          ))}
        </div>
...
function StrategiesScreen({ accent }) {
  const accentColor = ACCENTS[accent].main;
  return (
    <div style={{ padding: 20, height: '100%', overflow: 'auto' }}>
      ...
      <div style={{ display: 'grid', gap: 14, gridTemplateColumns: 'repeat(2, 1fr)' }}>
        {DATA.strategies.map(s => (
          <Card key={s.name}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontFamily: TOKENS.sans, fontSize: 15, fontWeight: 500, color: TOKENS.ink0, letterSpacing: '-0.02em' }}>{s.name}</span>
              <Pill tone="neutral">weight {(s.weight*100).toFixed(0)}%</Pill>
            </div>
            <Spark values={Array.from({length:12},(_,i)=>1+Math.sin(i*0.7)*0.2+i*0.05)} width={260} height={56} accent={accentColor}/>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginTop: 12, paddingTop: 12, borderTop: `1px solid ${TOKENS.line}` }}>
              <div>
                <Label style={{ marginBottom: 2 }}>Sharpe</Label>
                <span style={{ fontFamily: TOKENS.sans, fontSize: 16, fontWeight: 400, color: TOKENS.ink0 }}>{s.sharpe.toFixed(2)}</span>
              </div>
              <div>
                <Label style={{ marginBottom: 2 }}>Win rate</Label>
                <span style={{ fontFamily: TOKENS.sans, fontSize: 16, fontWeight: 400, color: TOKENS.ink0 }}>{(s.winRate*100).toFixed(0)}%</span>
              </div>
              <div>
                <Label style={{ marginBottom: 2 }}>Trades</Label>
                <span style={{ fontFamily: TOKENS.sans, fontSize: 16, fontWeight: 400, color: TOKENS.ink0 }}>{s.trades}</span>
              </div>
            </div>
          </Card>
        ))}
...
function TradeLogScreen() {
  ...
  return (
    <div style={{ padding: 20, height: '100%', overflow: 'auto' }}>
      ...
      <Card noPad>
        {rows.map((r, i) => (
          <div key={i} style={{ padding: '12px 18px', borderBottom: i < rows.length-1 ? `1px solid ${TOKENS.line}` : 'none', display: 'flex', alignItems: 'center', gap: 16, fontFamily: TOKENS.mono, fontSize: 11 }}>
            ...
            {r.loop && <span style={{ color: TOKENS.ink2 }}>loop #{r.loop} · path {r.path}</span>}
            <span style={{ flex: 1 }}/>
...
Object.assign(window, { SignalsScreen, BookScreen, RiskScreen, StrategiesScreen, TradeLogScreen });
