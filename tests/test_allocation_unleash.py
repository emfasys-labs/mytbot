...
def test_allocation_unleash_factors() -> None:
    ...
    # 2. Slider = 1.0 (deployment pressure relaxes shape only; safety stays live)
    ...
    assert dec_100.metadata.get("unleash_u") == 0.5
    assert dec_100.gross_exposure_target < Decimal("1.0")
    assert dec_100.gross_exposure_target > dec_90.gross_exposure_target
    ...
    assert dec_95.metadata.get("unleash_u") == 0.25
