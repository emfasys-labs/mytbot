...
- Last completed task: **D031 — Respect strategy sizing + sizing audit trail + execution-boundary guard**. Implemented the five phases (A-E) of the D031 brief. `signal_candidate_to_strategy_opportunity` now uses `metadata["risk_notional_override"]` > `metadata["target_notional"]` > `nav*position_pct` (fallback), clipped by `nav × max_position_pct` ceiling (never inflated upward). Full sizing audit trail (`sizing_source`, `sizing_*` fields) propagated through `StrategyOpportunity` → `CoordinatorAction` → `RawSignal`. New `ExecutionEngine._passes_sizing_boundary_guard` rejects orders > 1.25× intended notional. `held_positions_from_portfolio` flags oversized held positions. New `risk/stop_loss.py::evaluate_stop_loss` pure-function scaffold (not yet wired). Files: `portfolio/global_edge_coordinator.py`, `signals/arb_bridge.py`, `execution/engine.py`, `risk/engine.py` (docstring), `risk/stop_loss.py` (new), `system/trading_loop/loop.py`, `tests/test_global_edge_coordinator.py` (+7 cases), `tests/test_execution_engine.py` (+5 cases), `tests/test_stop_loss_scaffold.py` (new, 5 cases), `docs/DECISIONS.md` D031, `docs/ARCHITECTURE.md`. **369 tests pass.** **Known scope limitation (see D031 entry in DECISIONS.md):** strategies don't yet emit per-signal `target_notional`, so the live audit trail shows `sizing_source=nav_fallback` universally — the expected 7-13× size reduction materialises only after D032 makes strategies emit their own intent.
