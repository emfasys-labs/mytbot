...
- Last completed task: **D020 Dashboard heartbeat every tick** — `publish_dashboard_snapshot_heartbeat()` when full D015/global-edge publish does not run (`batch_candidates` empty, legacy path, or publish error); payload `heartbeat_only` + `dashboard_feed` (reason, counts); `SignalBrain` banner; `_run_global_edge_tick` returns `(executed, dashboard_written)`. Docs: `docs/DECISIONS.md` D020.
