...
- Last completed task: **D020 (amend)** — `GET /system/status` exposes **`trading.snapshot_published_at`** (mirrors last `dashboard.snapshot.updated_at`); UI **SNAPSHOT STALE** uses the newer of HTTP snapshot vs status timestamp. Heartbeat publish + `SignalBrain` banner unchanged. Docs: `docs/DECISIONS.md` D020.
