...
**Why move it (evidence, not assumption).** Built a read-only A/B harness `scripts/compare_ai_providers.py` that scores an identical 15-headline financial benchmark (earnings/macro/crypto/geo/M&A/regulatory/neutral) through each provider and reports directional-bias agreement, sentiment/confidence MAE, ticker overlap, JSON-validity, latency, and accuracy vs analyst-expected labels. Findings on this hardware: the local **`gpt-oss:20b`** took **~60–100s/call** (a single trivial probe = 98.6s) and **times out at its 20s production limit** — i.e. it was already contributing almost nothing live (D153 cooldown evidence). **Gemini 2.5 Flash (free tier):** **~3s/call (median 2.9s), 100% JSON validity (15/15), 93–100% directional accuracy** (the lone label "miss" was a defensible risk-off read of an ambiguous gold/banking-turmoil headline). Conclusion: switching is a strict **upgrade** in speed + reliability + structure, at **£0**, and frees the GPU so the system can run on a small free VM.

**Implementation (config-only on the live path + a tiny provider extension).** `ai/providers/local_reasoning_provider.py` already spoke OpenAI-compatible (any `/v1` base_url); added **API-key auth** (`api_key` / `api_key_env` / generic `LOCAL_REASONING_API_KEY` → `Authorization: Bearer`) and a **hosted model-trust** path (hosted gateways list models inconsistently, e.g. Gemini returns `models/gemini-2.5-flash`, so once the keyed endpoint is reachable we trust the configured model rather than gating on the installed-set membership check used for Ollama). **Fully backward-compatible** — keyless Ollama sends no header and behaves exactly as before. `config/ai.yaml::providers.local_reasoning` now points at `https://generativelanguage.googleapis.com/v1beta/openai`, model `gemini-2.5-flash`, `api_key_env: GEMINI_API_KEY`, `max_tokens: 2048` (2.5 is a *thinking* model — reasoning tokens draw from the output budget, so a tight cap starves the JSON; 2048 fixed the one benchmark JSON failure), `timeout_seconds: 30`, `gpu_concurrency: 2` (respect free-tier rate limits). The previous Ollama block is preserved **commented** in the YAML as an instant revert. Premium fallback (Anthropic) is unchanged; on Gemini failure/rate-limit the chain falls back to rules/FinBERT + premium.

**Caveats.** (1) Free-tier rate limits (~10 RPM / few-hundred RPD on Flash) are fine for necessity-gated escalation; on a 429 the chain degrades gracefully. (2) The supplied key has an `AQ.` prefix (works for `/models` + chat) — if it turns out to be an ephemeral/short-lived token rather than a permanent `AIza…` API key, `startup_check` will start failing and the layer falls back; replace with a permanent AI Studio key for 24/7 use. (3) Data: free-tier inputs may be used by Google to improve models — payloads are public news headlines + scores, not secrets.

**Status:** Implemented + **live after restart** — startup log `local_reasoning | hosted model active | model=gemini-2.5-flash … style=openai`; production immediately scored real news via `local_reasoning(gemini-2.5-flash)` (e.g. `KR` 3.4s, `MRNX` 4.7s) where the old Ollama logged `all models failed (20531ms)`. New: `scripts/compare_ai_providers.py`, `tests/test_local_reasoning_hosted.py` (6). Modified: `ai/providers/local_reasoning_provider.py` (api-key + hosted model trust), `config/ai.yaml` (Gemini block + commented Ollama revert), `.env` (`GEMINI_API_KEY`). Fully reversible (uncomment the Ollama block). Next: free cloud VM deploy (ARM Docker compose + IBC for IBKR).
...
**Shadow mode (`signals/engine.py`).** New `signal_engine.trained_meta_labeler_shadow` flag (default **true** now). When set, the engine loads the labeller config and **scores every signal**, stamping the would-keep/would-drop decision (`meta_label_probability`, `meta_label_threshold`, `meta_label_kept`, `meta_label_shadow: true`, model name/version) onto the signal metadata — but **never drops** the signal on it. Two distinct axes now: `use_trained_meta_labeler` (hard *enforce* = drop below threshold) vs `trained_meta_labeler_shadow` (*measure only*); enforce wins if both set. The refactor also made the labeller **always score** allocator-selected / operator-close signals (previously short-circuited) so the scoreboard sees the signals that actually became fills — but those exempt signals still pass `enforce=False`, so live behaviour is unchanged (they are never dropped). Requires `config/meta_labeler.yaml::trained_meta_labeler.enabled: true` (already set; model `mytbot_meta_labeler@0.2.0`, `approval_status: paper`) so the model actually scores.

**The brain scorecard (`scripts/report_brain_shadow.py`, read-only).** Joins the two sources of truth: SHADOW decisions (`signals` rows with `meta_label_shadow` + a real probability) vs REALISED outcome (the `fills` ledger). It reconstructs each `(broker, symbol)` position lifecycle into round-trip "streaks" (open→flat) from the signed `position_qty_after`, sums each streak's closing-fill `realised_pnl`, and attributes each round-trip to the nearest shadow entry decision (same symbol+side, at/just before the open). Buckets realised P&L into **would-KEEP** vs **would-DROP** and emits a verdict: **RE-ADMIT** (would-DROP round-trips were net-negative by a material margin AND would-KEEP ≥ would-DROP, with ≥ `min_sample` round-trips on both sides → filtering the dropped entries improves the book), **DO_NOT_ADMIT** (would-DROP were winners, or no clear gain), or **INSUFFICIENT_DATA**. Only a RE-ADMIT verdict justifies flipping `use_trained_meta_labeler: true`.

**Live first read:** after restart the shadow is producing **real** decisions — the v0.2.0 model scores ~0.13 prob (threshold ~0.5) and would-DROP exactly the recent churners (`USDJPY`, `CL=F`, `USO`, `XLE`), the same names the loss audit flagged. The scorecard correctly reports `INSUFFICIENT_DATA` until round-trips mature. Note the verdict gate is honest about a filter that would drop ~100% of flow: with no would-KEEP round-trips it can never reach RE-ADMIT (you cannot prove a filter that blocks everything), so a too-pessimistic model stays shadow-only by construction.

**Status:** Implemented + live after restart (5 brokers, `capital_pct=1.0`). New: `scripts/report_brain_shadow.py`, `tests/test_brain_shadow.py` (8). Modified: `signals/engine.py` (shadow axis + always-score refactor), `config/strategies.yaml` (`trained_meta_labeler_shadow: true`), `tests/test_arbitrage_loop_integration.py` (stub now honours the `enforce` kwarg). Full suite **1955 passed, 3 skipped**. Fully reversible (`trained_meta_labeler_shadow: false`). Next: let the shadow soak accrue, run `scripts/report_brain_shadow.py`, re-admit only on a RE-ADMIT verdict.

---

## D168 — Phase 3.2: profit-harvest horizon-aware anti-churn (close the last management-layer churn source)
**Date:** 2026-06-18
**Decision:** The post-D167 soak (~4h) confirmed the two D167.1/D167.2 bugs were fixed (proven longs fire; no new fractional futures), but exposed the *last* management-layer churn source that Phase 2 (D166) did not gate: `profit_harvest_monitor`. D166 gave the **stop-loss / aggregate-derisk / intraday-derisk** monitors a horizon-aware min-hold gate, but the profit-harvest monitor was left ungated. Live evidence: `XLE` opened 16:53 → **closed by profit-harvest at −$138 just 32 min later** (and `CL=F`/`USO` similar). Root cause: `evaluate_profit_harvest`'s `trailing_profit_lock` deliberately fires *into the red* (to protect a real +$3K→−$4K round-trip), but its peak-lock floor is only 0.05% of NAV — so a daily-horizon position that ticks up ~$600 then retraces gets closed at a small loss within the hour. On a *young* daily thesis that is pure churn tax, not profit-banking.

**Fix.** New pure, unit-tested guard `risk/profit_harvest.should_suppress_harvest_for_horizon()` and a wire into `system/orchestrator._run_profit_harvest_tick()` (reusing D166's `_protective_exit_config` + `_fills_age_seconds_by_symbol`). It suppresses **only** a `trailing_profit_lock` that would realise a **loss / immaterial gain** on a position younger than `protective_exit_min_hold.min_hold_sec` (3 days). Everything that genuinely banks edge is always allowed: `full_take_profit` / `partial_take_profit` (positive by construction), a trailing lock still locking **material positive** profit (≥ 0.10% NAV), and any harvest once the position has matured. Missing age → never suppress (no evidence to gate on, mirrors D166). Safety stop-loss / derisk paths are separate and unaffected (rule 2 intact — this is anti-churn, not a risk bypass).

**Status:** Implemented + live after restart. New tests: `tests/test_profit_harvest.py` (+6: young-loss-lock suppressed, full-TP allowed young, material-profit lock allowed, matured allowed, age-unknown allowed, no-reduce noop). Active because `config/risk_limits.yaml::protective_exit_min_hold.enabled: true`. Full suite **1955 passed, 3 skipped**. Fully reversible (`protective_exit_min_hold.enabled: false` disables it with the rest of D166). **Requires `python run.py` restart (done).**

---

## D167 — Phase 3: Maximize edge expression (un-gate the proven weapons; backfill universe breadth)
**Date:** 2026-06-18
**Decision:** Phase 3's mandate is "make the *proven* weapons actually fire and hold to thesis." D166 covered the *hold* half (anti-churn). This entry fixes the *fire* half. A live read-only funnel trace (`strategy_candidate_log`, 6h) found the proven long-horizon weapons were structurally suppressed before risk/execution by two blockers:

**1. The bug — stale macro-regime gate list (config + test).** `config/ai.yaml::pipeline.default_signal_strategies` (the YAML anchor every `regime_strategy_gates[regime]` points at, per D032) was **never updated when D158 added the two trend weapons**. So `AIPipeline.allowed_strategy_names(regime)` did not contain `trend_breakout` or `trend_following`, and `ai.regime.filter_by_allowed_strategies` (called in `candidate_collection.apply_regime_filter_with_logs`) **dropped 100% of their signals** with status `filtered_regime` / reason `macro_regime_gate` — before risk, before edge gate, before the orchestrator. The trace caught the single strongest weapon, `trend_breakout` (edge-gate PF **4.11**, 73% win, `allowed` full-size), getting **every one of its 22 long breakout setups gated out**. Fix: added `trend_breakout` + `trend_following` to `default_signal_strategies`; extended the D032 coverage test (`tests/test_ai_pipeline.py::...regime_gates...`) `required` set so this regression can never silently recur. (mean_reversion/momentum_breakout were already in the list and passing; the 5 parked unproven strategies stay listed but are `enabled:false` so they don't generate.)

**2. Breadth — daily-history starvation (data).** The proven long-horizon weapons need deep daily history (`trend_following` MA50/200 → 200 bars; `trend_breakout` L150 Donchian → 150 bars), but only **23 symbols** in the feature store had ≥200 daily bars (the original D157.1 backfill set) out of 1,399 with daily data — so those weapons could *only ever* fire on a tiny universe slice regardless of the gate. Launched a one-time **2y daily backfill across the active universe** (`core,scan` = 259 symbols) via the existing `run_pipeline.py --backfill --symbols-from-universe --universe-scope core,scan --skip-news --skip-fred` (idempotent feature_snapshots upserts; runs in parallel to the live loop, which reads new bars each tick). No code change — the standalone `--symbols-from-universe` flag already existed.

**Part C (investigated, not changed):** the `execution_precheck_rejected` / `order_status_rejected` on orchestrated equity opens (e.g. XLE) are transient IBKR-paper delayed-market-data liquidity-precheck rejections that **self-heal on retry** (XLE did open); already mitigated by the D140 paper liquidity overrides. Not a structural edge blocker — deliberately left untouched to avoid destabilizing the working precheck (rule: no regressions / no speculative hacks).

**Status:** Implemented + live after restart (all 5 brokers connected, `capital_pct=1.0`). Modified: `config/ai.yaml` (regime gate list), `tests/test_ai_pipeline.py` (coverage guard). Data op: universe daily backfill (background). Full suite **1933 passed, 3 skipped** (no regressions). Fully reversible (drop the two names from the list). **Requires `python run.py` restart (done).** Next: verify on the scoreboard that trend_breakout now passes the gate and round-trips, then Phase 4 (re-admit the shadowed brain, measured against the scoreboard) once soak data accrues.

---

## D167.1 / D167.2 — Phase 3.1: anti-churn ordering bug + futures whole-contract enforcement (post-deploy soak fixes)
**Date:** 2026-06-18
**Decision:** A post-D167 soak trace (zero fills in ~3h despite the regime gate now open) surfaced two concrete defects. Both fixed surgically; both proven by unit tests + full regression (**1941 passed, 3 skipped**). Live after restart.

**Bug 1 (D167.1) — an edge-gate-BLOCKED side suppressed a PROVEN side via anti-churn.** The signal engine's anti-churn cross-strategy contradiction gate (`signals/anti_churn.py`, D115) tombstones a symbol the moment it sees a long *and* a short on it. It ran (inside `raw_to_signal_candidate` / `process`) **before** the per-side edge-gate filter (`_apply_edge_gate_filter`, D161). Live effect on `USDJPY=X`: `mean_reversion` emitted a **short** that the edge gate had already certified `blocked` (mean_reversion#short PF<1, toxic per D161) — it could never trade — yet because anti-churn saw it first, it tombstoned the symbol and dropped `trend_breakout`'s **proven long** (PF 4.11) with `anti_churn:cross_strategy_contradiction`. The single best weapon literally could not trade its only setup. **Fix:** new `TradingLoop._filter_edge_gate_blocked_raws()` drops edge-gate-BLOCKED `(strategy, side)` RAW candidates **before** anti-churn records them, in both the D015 batch path and the legacy path (right after `apply_regime_weighting`). `reduced`/`allowed`/unproven-under-`reduce` sides pass through unchanged (their confidence scaling still happens later in `_apply_edge_gate_filter`); the drop is logged to the funnel as `edge_gate_blocked` with `metadata.stage=pre_anti_churn`. Net: a dead-on-arrival side can no longer poison a live one; genuine contradictions between two *tradeable* sides are still caught. Gated off when the edge gate is disabled (no-op).

**Bug 2 (D167.2) — fractional-contract futures fills from post-sizing notional clamps.** The signal engine already rounds futures to whole contracts (D165), but a **downstream** notional clamp (risk single-name room; crypto-venue room) recomputes `qty = clamped_notional / price` in **raw units**, silently producing a fractional contract. Observed live: `CL=F` (mult 1000) filled **313 units = 0.313 of a contract** after a risk clamp (`risk_notional_override` 23.2k / price 74.16), later topped up to a fractional **2313-unit** book. **Fix:** `ExecutionEngine._normalize_order_for_broker()` — the single chokepoint every order crosses after all clamps, before fill/submission — now rounds opening futures **DOWN to a whole multiple of the multiplier** (`futures_multiplier(symbol)`) and skips sub-contract opens (qty → 0, caught by the existing `qty<=0` guard → `invalid_quantity_after_normalization`). Reduce-only/close orders pass through unchanged so an existing fractional residual can still be flattened. This catches *every* clamp site at once (risk, crypto-venue, D125) instead of patching each. Note: the **routing** half is already correct (D165 `execution/router.py` pins exchange `=F` futures to IBKR and forbids crypto venues); the stale bybit `CL=F` paper position predates that and will be cleaned up by reduce-only exits.

**Verification:** `python -m py_compile execution/engine.py signals/engine.py system/trading_loop/loop.py`; new tests `tests/test_futures_execution_normalize.py` (5: sub-contract→0, fractional→floor, whole unchanged, reduce-only passthrough, equity control) + `tests/test_edge_gate_loop_filter.py` (+3: raw prefilter disabled-noop, drops-blocked-short-keeps-proven-long, keeps-unproven-reduce); full suite **1941 passed, 3 skipped** (only pre-existing torch/AsyncMock warnings). **Requires `python run.py` restart (done; auto-resumed running, `capital_pct=1.0`, 5 brokers).** Fully reversible. Next: confirm on the scoreboard that `trend_breakout` long round-trips and no new fractional-contract futures fills appear.

---

## D166 — Phase 2: Cost & Validation Discipline (horizon-aware anti-churn + cost-model reconciliation + scoreboard-gated trust)
**Date:** 2026-06-18
**Decision:** The D163 scoreboard proved the entire realised loss lived in the *management* layers (`stop_loss_monitor`, `aggregate_derisk`, `intraday_derisk`) — they cut **daily-horizon theses before they could mature** (median hold ~8.4h, 75% intraday) while the four proven daily weapons had **0 live closes**. Phase 2 stops that churn tax with three composable, fully-reversible pieces, without ever bypassing the risk engine (rule 2) and without removing catastrophic protection.

**Part A — Horizon-aware protective-exit gate (new pure module `risk/protective_exit_gate.py`).** A daily weapon needs days to be right; a stop/derisk that fires hours after entry on normal noise is churn, not protection. The gate suppresses **soft** protective exits for positions younger than `min_hold_sec` (default **3 days**), while *always* allowing genuine thesis-invalidation/survival exits. Position age is derived from the append-only `FillLog` (`position_age_seconds_from_fills` walks back to the start of the current open streak — no schema change). `should_suppress_protective_exit()` precedence: (1) gate disabled → never suppress; (2) catastrophic loss (≥ `catastrophic_loss_pct_nav` 4% of NAV **or** ≥ `catastrophic_loss_pct_position` 30% of cost basis) → fire; (3) structural ATR stop (thesis invalidated) → fire; (4) most-severe aggregate-derisk tier (portfolio survival) → fire; (5) otherwise, position younger than `min_hold_sec` → **suppress**; (6) matured → fire normally. Wired into all three orchestrator risk ticks (`_run_stop_loss_tick`, `_run_intraday_derisk_tick`, `_run_aggregate_derisk_tick`) — each fetches `position_ages` + the parsed config and skips (logs) suppressed actions. Config: `config/risk_limits.yaml::protective_exit_min_hold`.

**Part B — Cost-model reconciliation (the scoreboard now validates the backtest's cost assumption).** An `allowed`-but-live-net-negative weapon usually means the backtest cost model is too kind. `scripts/report_edge_scorecard.py` now loads the backtest `fee_bps`/`slippage_bps` (`config/strategies.yaml::backtest`, per-side → round-trip), and `cost_reconciliation()` compares live fee-drag + average |slippage| bps against it, flagging `BACKTEST_TOO_KIND` when live costs exceed the assumption beyond a tolerance band. A new "COST-MODEL RECONCILIATION" block in the report makes this explicit so backtest verdicts are only trusted when their cost basis matches reality.

**Part C — Scoreboard-gated strategy trust (live losers are never amplified).** `system/trading_loop/loop.py::_orchestrator_strategy_trust` already fused a backtest prior with a live-P&L posterior; Phase 2 adds an evidence threshold: the posterior is only applied once a strategy has ≥ `min_live_closes_for_posterior` (default **8**) live closing fills. Below that, trust = the a-priori edge prior only (don't react to noise). At/above it, a net-negative live strategy is **capped at `neutral` (1.0)** no matter how optimistic its backtest prior — capital can't flow *toward* a proven live loser — while net-positive live performance can still tilt up to `max_trust`. Config: `config/strategies.yaml::portfolio_orchestrator.min_live_closes_for_posterior`.

**Status:** Implemented + all three parts live after restart (all 5 brokers connected, `capital_pct=1.0`). New: `risk/protective_exit_gate.py`, `tests/test_protective_exit_gate.py`, `tests/test_orchestrator_strategy_trust.py`, `tests/test_edge_scorecard_cost.py`. Modified: `system/orchestrator.py` (3 risk ticks + `_fills_age_seconds_by_symbol` + `_protective_exit_config`), `system/trading_loop/loop.py` (`_orchestrator_strategy_trust` evidence gate), `scripts/report_edge_scorecard.py` (cost reconciliation), `config/risk_limits.yaml` (`protective_exit_min_hold`), `config/strategies.yaml` (`min_live_closes_for_posterior`). Tests: +28 new; full suite **1933 passed, 3 skipped**. Fully reversible: `protective_exit_min_hold.enabled: false` and `min_live_closes_for_posterior: 0` restore pre-Phase-2 behaviour. **Requires a `python run.py` restart (done).** Next: Phase 3 (maximise edge expression) once the scoreboard shows the proven core round-tripping cleanly under the anti-churn gate.

---

## D165 — Enable continuous-futures execution (front-month resolver, multiplier-aware boundary, IBKR-only routing)
**Date:** 2026-06-18
**Decision:** The proven daily weapons frequently fire their strongest signals on continuous futures (`CL=F`, `ES=F`, `NQ=F`, `GC=F`, …) because those symbols are in the universe, but futures execution had been gated off "until a contract-month resolver ships" (`FUTURES_EXECUTION_ENABLED=0`), so those edges produced **zero live closes**. The owner chose to **enable real futures execution** rather than exclude them. This is not a flag flip — a continuous `=F` symbol has no tradeable contract, carries an exchange-defined multiplier, consumes margin (not full notional), and can only trade on IBKR. This entry builds the full execution path safely.

**1. Instrument facts (`core/instruments.py`).** New `FuturesContractSpec` + `FUTURES_CONTRACT_SPECS` table (root → exchange + multiplier) for the CME/CBOT/COMEX/NYMEX/ICE roots in the universe (ES=50, NQ=20, CL=1000, GC=100, …). `futures_root` / `futures_spec_for` / `futures_multiplier` **require the `=F` suffix** so a futures root can never collide with an equity ticker (`CL`=Colgate, `GC`, `SI`).

**2. Front-month resolver (`brokers/ibkr/adapter.py`).** `_symbol_to_contract("CL=F")` returns a `ContFuture`; `_resolve_front_future()` qualifies it to a concrete front-month `Future` (e.g. `CLQ6`, July 2026), caches it (TTL + near-expiry rollover via `_futures_near_expiry`), and `_qualified_order_contract` uses it for orders. Verified live: `ContFuture(conId=304037475, symbol='CL', lastTradeDate='20260721', multiplier='1000', localSymbol='CLQ6')`.

**3. Multiplier-aware accounting (internal = notional-consistent units).** The system keeps futures quantity in notional-consistent UNITS (`contracts × multiplier`); all contract↔unit conversion happens at the IBKR boundary: order build divides units→whole contracts (`_build_ib_order_for_contract`, rejects <1 contract), fills/positions multiply contracts→units (`_trade_to_order_result`, `get_positions`), and `signals/engine.py` rounds futures sizing to whole contracts (drops sub-one-contract budgets). `system/trading_loop/helpers.py::broker_symbol_for` now **preserves** the `=F` suffix to keep symbols unambiguous to the adapter. Margin uses the pre-existing `future` cash factor 0.15.

**4. IBKR-only routing (the live bug this entry fixes).** First live run mis-routed `CL=F` to **Bybit** and produced a 671-fractional-"barrel" synthetic crypto fill: the router's `asset_class == "future"` carve-out conflated CME futures with Bybit **crypto perpetuals**, and IBKR's `future` permission was off, so Bybit was the only "future"-permitted venue. Fix (`execution/router.py`): any `=F` exchange future (`futures_spec_for(symbol) is not None`) pins to IBKR; if IBKR is not permitted/available it is **skipped** (return None), never falling through to a crypto venue. Crypto perps (no `=F`) still use Bybit. Enabled `ibkr.future` in `config/broker_permissions.yaml`.

**5. Multiplier-aware liquidity check (`brokers/ibkr/adapter.py::get_order_book`).** Second live blocker: the liquidity pre-check rejected `CL=F` (`liquidity=444.4 min=222180`) because IBKR reports futures depth in **contracts** (size 3 × price 74 = 444) while `order.quantity` is in units. Fixed at the adapter boundary — futures DOM/top-of-book sizes are scaled by the multiplier so depth notional (`3 × 1000 × 74 ≈ $444k`) compares like-for-like against the order notional; slippage estimation becomes consistent for free.

**Status:** Implemented + paper-validated end-to-end. Live result after restart: `CL=F` mean_reversion BUY (RSI 18 oversold) → routed IBKR → front-month `CLQ6` → sized `qty=3000` (3 contracts × 1000) → filled → position `ibkr CL=F qty=3000 ac=future avg=74.03` (≈$222k notional). Files: `core/instruments.py`, `brokers/ibkr/adapter.py`, `execution/router.py`, `signals/engine.py`, `system/trading_loop/helpers.py`, `config/broker_permissions.yaml`, `.env` (`FUTURES_EXECUTION_ENABLED=1`). Tests: +new `tests/test_futures_contract_spec.py`, `tests/test_futures_sizing.py`, `tests/test_ibkr_futures_execution.py`, futures-routing cases in `tests/test_router_permissions.py`, multiplier-scaling cases in `tests/test_ibkr_market_data_mode.py`; updated `tests/test_asset_class_routing.py` (suffix kept). Full suite **1905 passed, 3 skipped**. Reversible: set `FUTURES_EXECUTION_ENABLED=0` or `ibkr.future.enabled=false` (futures then cleanly skip — never mis-route). **Requires `python run.py` restart.** Note: IBKR paper serves only DELAYED futures market data (no real-time NYMEX subscription), so paper futures fills use local simulation at the resolved contract/price; this is correctly attributed to IBKR with proper contract sizing.

---

## D164 — Signal-derived sizing: edge-proportional Kelly trust + breadth-dynamic per-name cap
**Date:** 2026-06-18
**Decision:** The owner's standing principle is "no hard-coded absolute numbers — all parameters must be dynamic and depend on the entire system's signals (risk, market, opportunities)." D163's Phase 1 honoured *direction* but set static sizing constants (flat `max_position_pct_of_nav`, flat per-weapon `size_multiplier`). This entry makes the two sizing levers **derive their operating point from live signals**, keeping the config numbers only as **safety rails (bounds)** — the pattern the codebase already uses (D118 "no operator-tunable numbers, only safety bounds", D121 dynamic unleash, D132 market-state-scaled thresholds).

**1. Edge-proportional a-priori Kelly trust (capital flows to PROVEN edge).** The edge gate already measures each weapon's out-of-sample win-rate `W` and profit-factor `PF`; the classic Kelly stake collapses to a clean closed form with no magic constant: `f* = W·(PF−1)/PF` (`backtest/edge_gate.py::kelly_fraction`). New `EdgeGateRegistry.edge_kelly_trust()` computes `f*` for every certified verdict and normalises it onto the orchestrator's trust band — strongest proven weapon → `max_trust`, weaker proven weapons linearly between `neutral` and `max_trust` by their Kelly ratio (every proven weapon stays ≥ neutral — we tilt toward the best, never starve a profitable edge), blocked/adverse → `min_trust`. The trading loop folds this **a-priori** edge into the existing `strategy_trust` hook and multiplies it by the **posterior** recent-P&L tilt (`_orchestrator_strategy_trust(..., edge_prior=...)`): `trust = clamp(prior × posterior, min_trust, max_trust)`. The DECISIONS doc already framed the verdict as "the a-priori trust" and live P&L as "the posterior" — this finally wires the prior in. It only re-weights *between* weapons (gross is budgeted separately), so it cannot inflate deployment, and it does not touch the confidence gate (trust applies at orchestration, after candidate confidence filtering). Concretely on the live verdicts: trend_breakout (W .73, PF 4.11) → top trust; mean_reversion (W .64, PF 1.93) and momentum/trend_following get proportionally less — capital now follows the measured edge instead of being flat.

**2. Breadth-dynamic per-name cap (diversify when many edges fire, concentrate when few).** `portfolio/portfolio_orchestrator.py::orchestrate()` now derives the per-name cap each tick as `gross / number_of_firing_edges`, clamped to `[min_position_pct_of_nav, max_position_pct_of_nav]`. With many proven edges the book diversifies (cap shrinks below the ceiling); with one lone strong edge it concentrates up to the ceiling. The config `max_position_pct_of_nav` (0.20) is now the safety CEILING and the new `min_position_pct_of_nav` (0.02) the safety FLOOR — neither is the operating point. New diagnostics (`firing_breadth`, `per_name_frac`, `per_name_cap`, `strategy_trust`) surface the derived values so they are auditable, not hidden.

**Status:** Implemented. Files: `backtest/edge_gate.py` (`kelly_fraction`, `EdgeGateRegistry.edge_kelly_trust`), `portfolio/portfolio_orchestrator.py` (`min_position_pct_of_nav` bound + dynamic per-name cap + diagnostics), `system/trading_loop/loop.py` (`_orchestrator_strategy_trust` prior×posterior fusion + `_run_orchestrated_tick` edge-Kelly wiring), `config/strategies.yaml` (per-name cap reframed as rails + `min_position_pct_of_nav`). Tests: +8 (`tests/test_edge_gate.py` Kelly/trust-map, `tests/test_portfolio_orchestrator.py` breadth-cap + trust-tilt). Updated `tests/test_wave13_dashboard.py` for the D163 trained-meta-labeler shadow. Verification: `python -m pytest -q` → **1884 passed, 3 skipped**. Fully reversible (edge-Kelly returns `{}` and the per-name cap collapses to the ceiling if the new fields/gate are absent). **Requires a `python run.py` restart.**

---

## D163 — Profitability reset: the scoreboard + a clean, sanely-sized proven core
**Date:** 2026-06-18
**Decision:** The owner's verdict was that the system "makes losses a lot and doesn't earn much" despite having "a lot of brain." A full audit confirmed it and produced a phased plan; this entry implements **Phase 0 (the scoreboard)** and **Phase 1 (clean proven core, Kelly-rational sizing, shadow the unproven brain)** as one reversible block.

**Phase 0 — the scoreboard.** New read-only `scripts/report_edge_scorecard.py` joins the two sources of truth: BACKTEST edge (`data/state/edge_gate_verdicts.json` — per-strategy/per-side out-of-sample expectancy / PF / win rate) vs LIVE realised edge (the `fills` ledger — realised P&L, fees, holding period, win rate). It prints three blocks: an **expectancy scorecard** (backtest vs live side by side), a **cost ledger** (gross realised, ALL fees, NET, and fee-drag as bps of traded notional), and a **churn** report (closing-fill count, median hold, intraday-share). This is the instrument that tells us whether the foundation actually prints, instead of guessing.

**The "before" baseline it captured (pre-reset):** net realised **−$49,469**; the four proven daily weapons (trend_breakout/following, mean_reversion, momentum_breakout) had **0 live closes** — they never round-tripped — while the *entire* realised loss sat in the management layers: `stop_loss_monitor −$21.8k`, `aggregate_derisk −$13.9k`, `intraday_derisk −$12.8k`, `portfolio_orchestrator −$4.5k @ PF 0.14`, only `profit_harvest +$3.6k`. The orchestrator (predator mandate, D159) opened big at low conviction (median hold ~8.4h, **75% intraday**) and the de-risk machinery realised the damage before any *daily* thesis could mature. Diagnosis: the loss was sizing + churn driven, not edge driven.

**Phase 1 — clean proven core, Kelly-rational sizing (config-only, reverses D159/D159.1).** `config/strategies.yaml::portfolio_orchestrator` — `entry_conviction_threshold` 0.08→**0.15** (don't open marginal noise the de-risk layer must clean up), `flip_conviction_threshold` 0.40→0.45, `concentration_exponent` 2.5→**1.5** (diversify, no winner-take-all), `max_position_pct_of_nav` 1.00→**0.20** (per-name diversification), `gross_target_pct` 0.70/1.10/1.50 → **0.50/0.75/1.00** (deploy fully but UNLEVERED), `net_cap_pct_of_gross` 1.00→**0.70**. `config/risk_limits.yaml` — restored the real caps D159 removed: `single_name_notional.max_pct_nav` 1.00→**0.20**, `intraday_symbol_adds.max_pct_nav` 1.00→**0.20**, `min_signal_confidence` 0.30→**0.45**, `fx_cluster` 1.00→**0.25**, `equity_index_cluster` 1.00→**0.30**, `crypto_cluster` 1.00→**0.25**. D160 cluster *consolidation* stays on (one theme = one bet); the restored cluster caps now bound how big that one theme gets. **Shadowed the one clearly-unproven active brain layer:** `signal_engine.use_trained_meta_labeler` true→**false** (heuristic meta-labeler remains; edge gate + orchestrator are the proven controls). regime-weight quarantine and the heuristic meta-labeler stay (they *reduce* losses); the accumulator/demand/regime overlays are left for Phase 4 shadow-measurement against the scoreboard rather than ripped out blind.

**Clean baseline.** Wiped the paper trading ledger via the existing tested `scripts/reset_trading_data.py --execute` (truncated orders/positions/fills/daily_pnl/signals/risk_decisions/strategy_candidate_log + telemetry control_state, deleted risk_state.json/paper_wallet.json; KEEP feature_snapshots/price_history/model artefacts + `system.capital_allocation`). Restarted `python run.py`.

**Why this over "more brain":** profit comes from sizing PROVEN edge correctly and not paying a churn tax, not from leverage on noise. The scoreboard makes the next decisions evidence-based: after a soak, any weapon with live_pf>1 and net>0 earns more size; any `allowed`-but-net-negative weapon means the backtest cost model is too kind and gets investigated before being trusted.
**Status:** Implemented (config + one read-only script; fully reversible by restoring the D159/D159.1 values and `use_trained_meta_labeler: true`). New: `scripts/report_edge_scorecard.py`. Verification: both YAML files parse; `python -m pytest tests/test_risk_engine.py tests/test_portfolio_orchestrator.py tests/test_cluster_consolidation.py tests/test_fx_cluster_exposure.py tests/test_equity_index_cluster.py tests/test_d125_risk_caps.py tests/test_aggregate_derisk.py tests/test_edge_gate.py tests/test_edge_gate_loop_filter.py tests/test_meta_label_dynamic_threshold.py tests/test_intraday_derisk.py -q` (`175 passed, 1 skipped`). **Requires a `python run.py` restart (done).**
...
## D161 — Per-side edge verdicts + short-replay margin guard (edge is side-specific)
**Date:** 2026-06-09
**Decision:** The owner's review-fix pass added symmetric short replay to `backtest/harness.py`; running it revealed (a) edge is **side-specific** and (b) the short path could blow up without a margin bound. Implemented both fixes. (1) **Short-side margin guard** — a short entry is refused when its gross notional exceeds current equity (mirrors the long path's `total_cost <= cash`); without it, a losing high-frequency short sequence compounds without limit via the unpriced notional-as-quantity sizing fallback (volume_flow measured −$11.3B, volatility_regime −$1.3T — meaningless). Also added `allow_longs` so the harness can replay long-only / short-only / both. (2) **Per-side verdicts** — `scripts/run_edge_gate.py` now certifies each strategy twice: long-only under the canonical `name` key (back-compat with every existing reader) and short-only under `name#short`. `EdgeGateRegistry.is_blocked()/size_multiplier_for()/verdict_for()` accept `side=` (default "long"); the loop's `_apply_edge_gate_filter` resolves each candidate's entry side and blocks/reduces per side — sell-side ENTRIES from a short-blocked weapon are dropped, closes/reduce-only are unaffected (generated downstream from book state). (3) **Adverse-evidence rule** in `decide_verdict`: a thin sample (< min_trades) with *negative* measured expectancy is blocked even under the reduce policy (trend_breakout shorts: 25 trades, PF 0.34 — half-sizing that is just a slower bleed); zero-trade strategies keep cold-start reduce.
**Result (daily, 23 symbols, 15bps):** every proven weapon is **long-only** — momentum_breakout long PF 1.65 / short **BLOCKED** PF 0.37; mean_reversion long PF 1.93 / short **BLOCKED** PF 0.36 (−$124k); trend_following long PF 1.88 / short **BLOCKED** PF 0.59; trend_breakout long PF **4.11** / short blocked-by-adverse-evidence (25 trades, PF 0.34). volume_flow short (7 trades, PF 5.90) and volatility_regime short (PF 2.56 but consistency 0.39) are intriguing but uncertified. Canonical `data/state/edge_gate_verdicts.json` = the per-side daily run. The live effect after restart: the army takes long-side entries only until a short edge is proven; the prior short-USD forex book came from mean_reversion sell-side entries — the now-measured-toxic side. Tests: harness margin-guard/side-mode (3), registry per-side (3), loop per-side filter (2), adverse-evidence (2). Fixed the scripted-short test fixture to emit priced signals (its unpriced 20×-equity short was exactly what the guard must reject; the priced buy now also flips long, so the test asserts the flip). **Requires a `python run.py` restart.**

## D160 — Cluster-aware construction: express one theme as one big bet (not N fragments)
**Date:** 2026-06-09
**Decision:** After D159 removed the cluster CAPS, the orchestrator started opening many correlated positions that are really one bet — e.g. 5 forex pairs (AUDUSD/EURUSD long, USD-CAD/CHF/JPY short) that are all "short USD," fragmenting capital + conviction and paying 5× the spread for one economic trade. The fix is cluster *awareness* (NOT a cap): `portfolio/cluster_map.py` maps `(symbol, asset_class, side)` → `(cluster, theme_sign)` for three correlated families — `fx_usd` (USD direction from pair orientation, mirroring `RiskEngine._fx_pair_orientation`), `crypto_beta` (asset_class crypto or `-USD`/`USDT` suffix), `equity_index` (SPY/QQQ/IWM/DIA/VTI/… ETF set). `portfolio_orchestrator.consolidate_clusters()` runs before per-symbol netting: for each multi-member theme it sums the signed conviction in the theme's reference direction and emits ONE StrategyIntent on the strongest member, sized by the COMBINED conviction (capped at `cluster_conviction_cap`, default 1.5); single-member clusters and non-clustered symbols pass through untouched; opposing signals that net to zero express nothing. Verified on the exact live scenario: 5 short-USD pairs + GLD → one big EURUSD position at 100% of NAV (combined conviction 1.5) + GLD separate, instead of 5 forex fragments. Config: `portfolio_orchestrator.cluster_consolidation: true` + `cluster_conviction_cap`. Diagnostics add `raw_intent_count` / `clusters_consolidated`. The per-name and cluster CAPS stay removed (D159) — this is intelligence, not a limit; concentration is still uncapped. Tests: `tests/test_cluster_consolidation.py` (9). Full suite 1863 passed. **Requires a `python run.py` restart.**

## D159 — Predator mandate: unleash sizing (no per-name caps), aggressive deployment, drawdown breaker as the one floor
**Date:** 2026-06-09
**Decision:** Owner directive: the system was too conservative ("dead") — it should proactively fill the allocated capital, size dynamically by conviction with NO fixed per-name caps, and concentrate hard (up to 100% of NAV in one name) when a signal is overwhelmingly confirmed. Hard caps replaced by dynamic conviction-based sizing; the ONE remaining safety is a portfolio-level drawdown breaker (owner chose this over "fully unleashed"). Changes (config-only, reversible): **risk_limits.yaml** — `single_name_notional.max_pct_nav` 0.05→**1.00** (a single name can take 100% of NAV; the 1.00 ceiling is now only a sanity guard against sizing *bugs*, not a real limit), `intraday_symbol_adds.max_pct_nav` 0.10→1.00 (full intraday rotation into one name), `max_daily_loss_pct` 0.02→0.05, `max_drawdown_pct` 0.10→**0.20** (THE survival breaker — de-risk/halt only if whole-book drawdown from HWM > 20%), `min_signal_confidence` 0.50→0.30 (gates SIZE not pass/fail — the orchestrator floors order confidence at 0.30 so this no longer blocks opportunities; weak signals still act but conviction-sizing keeps them small). **strategies.yaml::portfolio_orchestrator** — `max_position_pct_of_nav` 0.05→**1.00** (no per-name cap), `gross_target_pct` defender/trader/hunter 0.50/0.90/1.30 → **0.70/1.10/1.50** (fill the capital; allow leverage), `net_cap_pct_of_gross` 0.60→**1.00** (fully directional — back up the truck one way), `concentration_exponent` 1.5→**2.5** (winner-take-all), `entry_conviction_threshold` 0.15→0.08 (act on most opportunities). Verified: a 2-weapon-confirmed BTC signal now sizes to **100% of NAV** while weak signals get 1% slivers and the book deploys ~101%+. **Deliberate design call — KEEP winners, don't literally "sell everything":** the orchestrator funds a new dominant bet from the (now-levered) gross budget and by closing LOSING/weak holds, but it still rides PROFITABLE/young positions (esp. the proven PF-4.02 trend snipers, which fire once and need to hold). Force-closing winners to fund another bet is the exact 0%-win rotation bleed that D156 was built to stop, and is *less* profitable than holding the winner AND adding the new bet via the deployment budget. What stays: the edge gate (targeting computer — go big only on weapons proven to hit) and the drawdown breaker (survival). **Honest caveat:** sizing amplifies edge in BOTH directions; aggression multiplies the modest proven daily edges (PF 1.3–4) into bigger swings — more upside and more downside — bounded only by the 20% drawdown breaker. Full suite 1854 passed. **Requires a `python run.py` restart to take effect.**

**D159.1 — Remove the cluster caps too (the next layer of strangleholds) (2026-06-09):** After the first restart, diagnosis of the live candidate log (carefully separating *current* from *stale* pre-restart rows via the DB clock) showed the per-name cap was correctly off, but the orchestrator's aggressive forex orders were now blocked by the D115/D131 **cluster caps** and the theme gate: `fx_cluster.max_usd_directional_exposure_pct` 0.15→**1.00**, `equity_index_cluster.max_net_exposure_pct` 0.20→**1.00**, `crypto_cluster.max_net_exposure_pct` 0.10→**1.00** (the 0.10 crypto cap alone would have blocked the headline "100% Bitcoin" scenario at 10%), `theme_uniqueness_check` true→**false**. These were the actual binding constraints once the per-name cap was lifted — a forex-concentrated book hit the 15% FX-cluster ceiling. What stays: execution-quality gates (max_slippage/max_spread — don't trade illiquid garbage), the separate arbitrage-stack caps, and the 20% drawdown breaker. Tests 85 passed (cluster/risk subset). **Requires another `python run.py` restart.**
...
**D158 Phase 3 — Multi-horizon trend snipers (weekly/monthly) (2026-06-07):** Key insight: a trend weapon's HORIZON is set by its lookback + holding period, not the bar timeframe — a 150-day Donchian breakout held for months is a "monthly sniper" that runs in the existing daily loop on the data we already have (no new pipeline/loop). Validated with `scripts/sweep_trend_horizons.py` (walk-forward over the 23-symbol daily universe, realistic costs): the trend edge strengthens monotonically with horizon — trend_breakout L50→L100→L150→L200 gave PF 2.22→3.03→**4.15**→4.04 (win 64%→69%→**73%**→73%), trend_following MA20/50→50/100→50/200 gave PF 1.23→2.11→2.46. This is the textbook long-horizon time-series-momentum anomaly. Retuned the two live trend weapons to their proven-best long horizons: `trend_breakout` entry_lookback 50→**150** (monthly sniper), `trend_following` MA 20/50→**50/200** (monthly trend). Also bumped `backtest.max_hold_bars` 20→60 (the old 20 was an intraday-era cap that truncated long trends; daily weapons hold weeks-months). Re-ran the edge gate: **all four core weapons now ALLOWED at full size** — trend_breakout (96 trades, +$79.6k, **PF 4.02**, 73% win — strongest in the arsenal), trend_following (297 trades, +$94.5k, PF 1.98 — promoted from REDUCED), mean_reversion (PF 1.94), momentum_breakout (PF 1.65). The army now spans cadences: momentum/mean_reversion (days) → trend_following (weeks-months) → trend_breakout (months). Canonical daily verdicts updated. Full suite 1852 passed.

**D158 Phase 3.1 — Trend-hold polish + multi-instance enabler (2026-06-07):** Closed the two deferred refinements. (1) **Continuation/exit signal** — added a config-gated `hold_with_trend` mode to `trend_breakout` (emits a lower-conviction continuation while price is on the trending side of the slow-channel midpoint, flips on reversal). Validated on the daily universe: continuation makes more *total* profit (+$77.8k vs +$53.9k) but at far lower quality (PF 1.89 / 52% win vs **4.15 / 73%** for breakout-only) — it whipsaws around the midpoint. So it is kept **default OFF**; breakout-only is the higher-quality sniper. (2) **Live-hold fix in the orchestrator instead** — the real reason a backtest-proven sniper could underperform live is the orchestrator closing a fresh entry on day-2 noise (flat-desire close of a not-yet-profitable position). Extended the flat-desire edge-protection to also hold positions younger than `min_hold_sec_before_flip`, and set that to **3 days** (259200s; was 1800s, an intraday-era value) so fresh entries ride through early noise without the whipsaw of an always-on continuation signal. (3) **Multi-instance enabler** — `strategies/base.py` now honours an optional `strategy_name` config override, so the same class can run as several distinct weapons at different horizons (e.g. `trend_breakout_weekly` vs `_monthly`) via config alone. Did NOT populate redundant instances — the multi-horizon spread is already achieved across weapon *types* (momentum fast → MR knife → trend_following weeks → trend_breakout months); adding a 2nd correlated breakout is low marginal value, now unblocked for when it's wanted. Tests: 2 new orchestrator (young-flat hold/close). Full suite **1854 passed**. The 4-weapon daily army is now complete and ready to paper-soak.

**D158 Phase 2 — Heterogeneous risk modes (global throttle × per-weapon temperament) (2026-06-07):** Redefined Defender/Trader/Hunter from one global switch into two composable axes. (1) **Global threat throttle** — the existing `system/adaptive_mode.py` mode (defender/trader/hunter, driven by drawdown + volatility) maps to a systemic threat scalar via `mode_threat` (hunter 0.0 / trader 0.35 / defender 0.75). (2) **Per-weapon temperament** — each weapon has an intrinsic style (`weapon_temperament`: trend_breakout=sniper, trend_following/momentum=shotgun, mean_reversion=knife), each with a `size_mult` (sniper 1.30 sizes bigger per shot, knife 0.75 smaller) and `defensive_cut` (sniper 0.30 most resilient, knife 1.00 cut hardest). Composed in `portfolio/portfolio_orchestrator.py`: a weapon's effective conviction = raw × edge_trust × `temperament_factor = size_mult × (1 − threat × defensive_cut)`. In calm markets the army is heterogeneous by style; in danger everyone retreats *together but unevenly* — sniper stays ~1.01, shotguns ~0.55, knife collapses to ~0.19 (and can drop below the entry bar entirely, benching itself). New `OrchestratorConfig` fields (`temperaments`, `weapon_temperament`, `mode_threat`) + `threat_for()`/`temperament_factor()`; diagnostics expose `mode`/`threat_level`/`temperament_factors` so the operator can see the posture. Empty config → all factors 1.0 (backward compatible). Loop wiring fixed to use `dataclasses.replace` so the orchestrator config carries all fields (was field-by-field, dropping new ones). Config block: `config/strategies.yaml::portfolio_orchestrator.{temperaments,weapon_temperament,mode_threat}`. Tests: 7 new in `tests/test_portfolio_orchestrator.py` (24 total). Full suite 1852 passed. **Answers the user's question — the army does NOT uniformly become defenders: systemic danger throttles everyone, but regime/temperament keeps it heterogeneous; individual edge-health (edge gate + live P&L trust) benches malfunctioning weapons independently. Next: Phase 3 — weekly/monthly snipers.**
